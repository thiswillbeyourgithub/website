...
*Individual project count on this page: 119*
...
*26 projects so far*
- [justelesRCP](https://justelesrcp.olicorne.org): a fast, ad-free static site serving the official French drug summary sheets (RCP, résumés des caractéristiques du produit) built from the public ANSM/BDPM open data. Roughly 15,600 medications, each a precomputed page with instant client-side search, no ads, no accounts, no tracking, served from a security-hardened, read-only Caddy container and hosted in France. There's a public instance at [justelesrcp.olicorne.org](https://justelesrcp.olicorne.org) and the source is on [GitHub](https://github.com/thiswillbeyourgithub/justelesRCP).
